...
class BaristaOrderHistoryScreen extends StatefulWidget {
  const BaristaOrderHistoryScreen({super.key});

  @override
  State<BaristaOrderHistoryScreen> createState() =>
      _BaristaOrderHistoryScreenState();
}

class _BaristaOrderHistoryScreenState extends State<BaristaOrderHistoryScreen> {
      ...
      final List<Map<String, dynamic>> mockOrders = [
        {
          'id': '15',
          'mesa': '7',
          'hora': timeFormatter.format(now.subtract(const Duration(hours: 1))),
          'fecha': formatter.format(now),
          'estado': 'completado',
          'tiempo_preparacion': '8 min',
          'bebidas': [
            {'nombre': 'Café Americano', 'cantidad': 2},
            {'nombre': 'Smoothie de mango', 'cantidad': 1},
          ],
        },
        {
          'id': '14',
          'mesa': '3',
          'hora': timeFormatter.format(now.subtract(const Duration(hours: 2))),
          'fecha': formatter.format(now),
          'estado': 'completado',
          'tiempo_preparacion': '6 min',
          'bebidas': [
            {'nombre': 'Té chai', 'cantidad': 1},
            {'nombre': 'Expresso', 'cantidad': 1},
          ],
        },
        {
          'id': '10',
          'mesa': '5',
          'hora': '10:30',
          'fecha': formatter.format(now.subtract(const Duration(days: 1))),
          'estado': 'completado',
          'tiempo_preparacion': '12 min',
          'bebidas': [
            {'nombre': 'Café Mocha', 'cantidad': 1},
            {'nombre': 'Chocolate caliente', 'cantidad': 2},
          ],
        },
        {
          'id': '8',
          ...
          'bebidas': [
            {'nombre': 'Latte con vainilla', 'cantidad': 2},
          ],
        },
        {
          'id': '5',
          'mesa': '8',
          'hora': '11:20',
          'fecha': formatter.format(now.subtract(const Duration(days: 2))),
          'estado': 'completado',
          'tiempo_preparacion': '9 min',
          'bebidas': [
            {'nombre': 'Capuccino', 'cantidad': 2},
            {'nombre': 'Té de manzanilla', 'cantidad': 1},
          ],
        },
      ];
      ...
      children: [
        ...
        Padding(
          ...
          child: Row(
            children: [
              Text(
                '${filteredOrders.length} bebidas completadas',
  ...
  Widget _buildOrderHistoryCard(
    ...
    return Card(
      ...
      child: ExpansionTile(
        ...
        leading: Icon(Icons.coffee, color: theme.colorScheme.primary, size: 28),
        title: Row(
          children: [
            Text(
              'Pedido #${order['id']}',
        ...
        children: [
          const Divider(),
          Text('Bebidas preparadas:', style: theme.textTheme.titleSmall),
          const SizedBox(height: 8),
          ...order['bebidas']
              .map<Widget>(
                (bebida) => Padding(
